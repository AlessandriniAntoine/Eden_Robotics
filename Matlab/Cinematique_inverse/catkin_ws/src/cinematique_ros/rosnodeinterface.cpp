...
// Generated 25-Nov-2021 11:42:41
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"Cinematique_ROS\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(Cinematique_ROS_M, (NULL));
        Cinematique_ROS_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(Cinematique_ROS_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(Cinematique_ROS_M)
                == (NULL)) || rtmGetStopRequested(Cinematique_ROS_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(Cinematique_ROS_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
