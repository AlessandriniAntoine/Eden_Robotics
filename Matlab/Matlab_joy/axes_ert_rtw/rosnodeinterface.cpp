...
// Generated 08-Feb-2022 03:18:00
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"axes\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(axes_M, (NULL));
        axes_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(axes_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(axes_M)
                == (NULL)) || rtmGetStopRequested(axes_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(axes_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
